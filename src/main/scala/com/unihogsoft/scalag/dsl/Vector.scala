...
trait Vector {
  ...
  case class MulVecScalar[T <: ScalarType, R <: VectorType[T] : WeakTypeTag](vec: E[R], scalar: E[T]) extends Expression[R]
  trait MulVecScalarOp {
    self: VectorType[_ <: ScalarType] =>
    type Arb <: ScalarType
    def *(other: self.ElemType)(implicit ct: WeakTypeTag[self.Self], ev: other.Self =:= self.ElemType): Self = {
      self.init(MulVecScalar(
        self.tree.asInstanceOf[E[VectorType[Arb]]],
        other.tree.asInstanceOf[E[Arb]]
      )(implicitly[WeakTypeTag[self.Self]].asInstanceOf[WeakTypeTag[VectorType[Arb]]])
  ...
  case class DivVecScalar[T <: ScalarType, R <: VectorType[_] : WeakTypeTag](vec: E[R], scalar: E[T]) extends Expression[R]
  ...
  case class DotProd[T <: ValType : WeakTypeTag](a: E[T], b:  E[T]) extends E[T]
  trait DotProdable {
    self: ValType =>
    def dot(other: self.Self)(implicit ct: WeakTypeTag[self.Self]): Self = biCombine(DotProd(_, _), other)
  }

  case class CrossProd[T <: ValType : WeakTypeTag](a: E[T], b:  E[T]) extends E[T]
  trait CrossProdable {
    self: ValType =>
    def cross(other: self.Self)(implicit ct: WeakTypeTag[self.Self]): Self = biCombine(CrossProd(_, _), other)
  }
