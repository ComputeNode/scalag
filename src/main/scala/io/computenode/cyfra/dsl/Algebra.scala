...
object Algebra:
  ...
  extension[T <: Scalar: ScalarSummable : Tag](a: T)
    @targetName("add")
    inline def +(b: T): T = summon[ScalarSummable[T]].sum(a, b)
  ...
  extension[T <: Scalar: ScalarDiffable : Tag](a: T)
    @targetName("sub")
    inline def -(b: T): T = summon[ScalarDiffable[T]].diff(a, b)
  ...
  extension [T <: Scalar : ScalarMulable : Tag](a: T)
    @targetName("mul")
    inline def *(b: T): T = summon[ScalarMulable[T]].mul(a, b)
  ...
  extension [T <: Scalar : ScalarDivable : Tag](a: T)
    @targetName("div")
    inline def /(b: T): T = summon[ScalarDivable[T]].div(a, b)
  ...
  extension [T <: Scalar : ScalarNegatable : Tag](a: T)
    @targetName("negate")
    inline def unary_- : T = summon[ScalarNegatable[T]].negate(a)
  ...
  extension [T <: Scalar : ScalarModable : Tag](a: T)
    inline def mod(b: T): T = summon[ScalarModable[T]].mod(a, b)
  ...
  extension [T <: Scalar : Comparable : Tag](a: T)
    inline def >(b: T): GBoolean = summon[Comparable[T]].greaterThan(a, b)
    inline def <(b: T): GBoolean = summon[Comparable[T]].lessThan(a, b)
    inline def >=(b: T): GBoolean = summon[Comparable[T]].greaterThanEqual(a, b)
    inline def <=(b: T): GBoolean = summon[Comparable[T]].lessThanEqual(a, b)
    inline def ===(b: T): GBoolean = summon[Comparable[T]].equal(a, b)

  case class Epsilon(eps: Float)
  given Epsilon = Epsilon(0.00001f)
  
  extension (f32: Float32)
    inline def asInt: Int32 = Int32(ToInt32(f32))
    inline def =~=(other: Float32)(using epsilon: Epsilon): GBoolean =
      abs(f32 - other) < epsilon.eps
  
  extension (i32: Int32)
    inline def asFloat: Float32 = Float32(ToFloat32(i32))
    inline def unsigned: UInt32 = UInt32(ToUInt32(i32))
    
  extension (u32: UInt32)
    inline def asFloat: Float32 = Float32(ToFloat32(u32))
    inline def signed: Int32 = Int32(ToInt32(u32))
  ...
  extension[V <: Vec[_] : VectorSummable : Tag](a: V)
    ...
    inline def +(b: V): V = summon[VectorSummable[V]].sum(a, b)
  ...
  extension[V <: Vec[_] : VectorDiffable : Tag](a: V)
    ...
    inline def -(b: V): V = summon[VectorDiffable[V]].diff(a, b)
  ...
  extension[S <: Scalar : Tag, V <: Vec[S] : Tag](s: S)(using VectorScalarMulable[S, V])
    def *(v: V): V = summon[VectorScalarMulable[S, V]].mul(v, s)
  ...
  extension[T <: Scalar : BitwiseOperable : Tag](a: T)
    inline def &(b: T): T = summon[BitwiseOperable[T]].bitwiseAnd(a, b)
    inline def |(b: T): T = summon[BitwiseOperable[T]].bitwiseOr(a, b)
    inline def ^(b: T): T = summon[BitwiseOperable[T]].bitwiseXor(a, b)
    inline def unary_~ : T = summon[BitwiseOperable[T]].bitwiseNot(a)
    inline def <<(by: UInt32): T = summon[BitwiseOperable[T]].shiftLeft(a, by)
    inline def >>(by: UInt32): T = summon[BitwiseOperable[T]].shiftRight(a, by)
  ...
  inline def toFloat32(f: FloatOrFloat32): Float32 = f match
  ...
  extension [T <: Scalar: FromExpr: Tag] (v2: Vec2[T])
    inline def x: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v2, Int32(ConstInt32(0))))
    inline def y: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v2, Int32(ConstInt32(1))))
    
  extension [T <: Scalar: FromExpr: Tag] (v3: Vec3[T])
    inline def x: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v3, Int32(ConstInt32(0))))
    inline def y: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v3, Int32(ConstInt32(1))))
    inline def z: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v3, Int32(ConstInt32(2))))
    inline def r: T = x
    inline def g: T = y
    inline def b: T = z

  extension [T <: Scalar: FromExpr: Tag] (v4: Vec4[T])
    inline def x: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v4, Int32(ConstInt32(0))))
    inline def y: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v4, Int32(ConstInt32(1))))
    inline def z: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v4, Int32(ConstInt32(2))))
    inline def w: T = summon[FromExpr[T]].fromExpr(ExtractScalar(v4, Int32(ConstInt32(3))))
    inline def r: T = x
    inline def g: T = y
    inline def b: T = z
    inline def a: T = w
    inline def xyz: Vec3[T] = Vec3(ComposeVec3(x, y, z))
    inline def rgb: Vec3[T] = xyz
  ...
  def vec2(x: FloatOrFloat32, y: FloatOrFloat32): Vec2[Float32] =
  ...
  def vec2(f: FloatOrFloat32): Vec2[Float32] = (f, f)
  ...
  inline def vclamp(v: Vec3[Float32], min: Float32, max: Float32): Vec3[Float32] =
