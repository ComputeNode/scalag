...
object Functions:
  ...
  def max(f1: Float32, f2: Float32, fx: Float32*): Float32 = fx.foldLeft(max(f1, f2))((a, b) => max(a, b))
  def max[V <: Vec[Float32] : Tag : FromExpr](v1: V, v2: V): V =
    summon[FromExpr[V]].fromExpr(FunctionCall(Max, List(v1, v2)))
  def max[V <: Vec[Float32] : Tag : FromExpr](v1: V, v2: V, vx: V*): V =
    vx.foldLeft(max(v1, v2))((a, b) => max(a, b))

  case object Min extends FunctionName
  def min(f1: Float32, f2: Float32): Float32 = Float32(FunctionCall(Min, List(f1, f2)))
  def min(f1: Float32, f2: Float32, fx: Float32*): Float32 = fx.foldLeft(min(f1, f2))((a, b) => min(a, b))
  def min[V <: Vec[Float32] : Tag : FromExpr](v1: V, v2: V): V =
    summon[FromExpr[V]].fromExpr(FunctionCall(Min, List(v1, v2)))
  def min[V <: Vec[Float32] : Tag : FromExpr](v1: V, v2: V, vx: V*): V =
